    ...
    def update_mode(change=None):
        ...
        match state['mode']:
            ...
            case 'Grad-CAM++' | 'Score-CAM' | 'XGrad-CAM' | 'FullGrad' | 'Ablation-CAM' | 'Layer-CAM' | 'Shapley-CAM':
                ...
                match state['mode']:
                    case 'Grad-CAM++':
                        cam = get_cam_comparative(state['model'], state['input_tensor'], 'gradcampp')
                    case 'Score-CAM':
                        cam = get_cam_comparative(state['model'], state['input_tensor'], 'scorecam')
                    case 'XGrad-CAM':
                        cam = get_cam_comparative(state['model'], state['input_tensor'], 'xgradcam')
                    case 'FullGrad':
                        cam = get_cam_comparative(state['model'], state['input_tensor'], 'fullgrad')
                    case 'Ablation-CAM':
                        cam = get_cam_comparative(state['model'], state['input_tensor'], 'ablation')
                    case 'Layer-CAM':
                        cam = get_cam_comparative(state['model'], state['input_tensor'], 'layercam')
                    case 'Shapley-CAM':
                        cam = get_cam_comparative(state['model'], state['input_tensor'], 'shapleycam')
                print(f"cam shape:{cam.shape}, cam dtype: {cam.dtype}, cam device: {cam.device}")
